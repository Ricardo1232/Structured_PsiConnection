...
def predict(symptoms):
    ...
    with torch.no_grad():
        ...
        diagnoses_p, diagnoses_s = calcular_porcentajes(probabilities, etq_enfermedades, enfermedades, symptoms)

        return diagnoses_p, diagnoses_s
    
def calcular_porcentajes(probabilities, etq_enfermedades, enfermedades, sintomas):
    predictions = {}
    for i, prob in enumerate(probabilities):
        enfermedad = etq_enfermedades[i]
        sintomas_enfermedad = enfermedades[enfermedad]
        sintomas_presentes = [symptom for symptom in sintomas if symptom in sintomas_enfermedad]
        
        # Calcular fracción de síntomas presentes
        fraccion_sintomas = len(sintomas_presentes) / len(sintomas_enfermedad) if len(sintomas_enfermedad) > 0 else 0
        # Asegurar que el porcentaje no exceda el 100%
        porcentaje = min(prob * fraccion_sintomas * 100, 100)
        
        predictions[enfermedad] = porcentaje
...
archivo = r"ia/test/txt/test_red.txt"
