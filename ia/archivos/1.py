...
def generar_datos():
    ...
    X_train, y_train = all_enfermedades()
    print(X_train)
    print(len(X_train))
    # X_train_all, y_train_all = all_enfermedades()
    
    # Generar datos aleatorios
    # X_train_random, y_train_random = generar_datos_random(5000)
    
    # Generar datos sintéticos
    # X_train_sinteticos, y_train_sinteticos = generar_datos_sinteticos(enfermedades, num_samples=20000)
    # X_train, y_train = generar_datos_sinteticos(enfermedades, num_samples=20000)
    
    # Combinar todos los datos
    # X_train = np.concatenate((X_train_all, X_train_sinteticos), axis=0)
    # y_train = np.concatenate((y_train_all, y_train_sinteticos), axis=0)
...
def entrenar_modelo():
    ...
    modelo_guardado = r'ia/modelo/modelo_enfermedades.h5'
...
if __name__ == "__main__":
    ...
    ['nerviosismo', 'fatiga', 'problemas_concentracion', 'irritabilidad', "desprecio_normas_sociales", "manipulacion_engano", "comportamiento_impulsivo_agresivo", "enfado", "ansiedad", "nauseas", "sonidos_desencadenantes"]
    ]
    model = crear_modelo(len(todos_los_sintomas), len(enfermedades))
    modelo_guardado = r'ia/modelo/modelo_enfermedades.h5'
