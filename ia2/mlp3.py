...
def generar_datos(num_muestras):
    ...
    for i in range(num_muestras):
        ...
        if caso == 'normal':
            ...
            X[i] = np.random.choice([0, 1, 2], size=NUM_PREGUNTAS, p=[0.85, 0.10, 0.05])
            ...
            X[i, inicio:fin] = np.random.choice([1, 2], size=PREGUNTAS_POR_TRASTORNO, p=[0.2, 0.8])
            # Respuestas bajas en las demás preguntas
            mask = np.ones(NUM_PREGUNTAS, dtype=bool)
            mask[inicio:fin] = False
            X[i, mask] = np.random.choice([0, 1, 2], size=np.sum(mask), p=[0.85, 0.10, 0.05])
        
        elif caso == 'comorbid':
            # Comorbilidad: 2 o 3 trastornos positivos
            ...
            # Respuestas mezcladas de manera aleatoria
